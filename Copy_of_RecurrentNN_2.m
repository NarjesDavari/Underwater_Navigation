...
% x_traning_set=Real_Measurement.DVL_outlieradded_manually(1:5900,2);
% x_testing_set=Real_Measurement.DVL_outlieradded_manually(1:5900,2);
x_traning_set=Real_Measurement.DVL(:,2); %%(6000:9500,2); %%(2200:4700,2); %%(3400:3800,2); %% 
x_testing_set = Real_Measurement.DVL(:,2); %%(6000:9500,2);
...
alpha_hu = 0.001; alpha_bhu=0.001; alpha_ih=0.001;alpha_bih=0.001; alpha_hh=0.001;alpha_uu=0.001; alpha_buu=0.001;
beta_coeff1 = 0.9; beta_coeff2=0.9;
...
%% 
    ...
    I_testing_set = x_testing_set(i:w+i-1);
    I_traning_set = x_traning_set(i:w+i-1);
%     if i>2
%         I_traning_set(end)= u(end);
%     end
...
%             y1 = log(1+exp(W_ih_new .* I_traning_set + b_ih_new + W_hh_new .* h));
        ...
        energ_test(i,kk) = abs(0.5 * norm((I_traning_set-u_hat)).^2);
        partition_f = 1;
        ...
        recon_error (i,kk)=u(end)-u_hat(end);
         recon_error_sum (i,kk)=sum(u(end)-u_hat(end));
...
kk= kk+1;       
    ...
    if kk == k1*5
       for k=2:w+1
        ...
        h = h(2:end,1);
        u_hat = ActiveF_diff(W_hu .* h + b_hu,1); %%%logsig(h_i); %% output from neuron of hidden layer
        E_testing(k1+1,1) = exp(0.5* sum((u-u_hat).^2));
        E_testing_min = min(E_testing(2:k1+1));
        GL = 100*(E_testing(k1+1,1)/E_testing_min-1); %%% Generalization Loss
        if GL >1 
          flag_Learning =0;
        end
        k1=k1+1;
...
score(i,1) = mean(energ(i,2:end));
energ_timeW (i,1) = mean (energ(i,2:end));
energ_timeW2 (i,1) = mean (energ_test(i,2:end));

if i>2*w+1
    sd =std(energ_timeW (i-2*w-1:i-1,1));
    thr_up = median(energ_timeW(i-2*w-1:i-1,1)) +0.1*sd;
    thr_down = median(energ_timeW(i-2*w-1:i-1,1)) -0.1*sd; 
    med_up(i,1) = median(energ_timeW(i-2*w-1:i-1,1));
    sd_up(i,1)= sd;
    thresh_down(i,1) = thr_down;%%median
    
    if energ_timeW(i,1)> thr_up || energ_timeW(i,1)< thr_down  %%median
        x_out(i,1) = x_traning_set (i-1);
    else
        x_out(i,1) = x_traning_set (i);
    end
else
    x_out(i,1) = x_traning_set (i);
end
